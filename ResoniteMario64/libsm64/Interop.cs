...
public static class MarioExtensions {
    ...
    public static float3 ToMarioPosition(this float3 pos) {
        return Interop.SCALE_FACTOR * pos * new float3(-1, 1, 1);
    }
}
...
internal static class Interop {
    ...
    public struct SM64MarioState {
        ...
        public float3 UnityPosition =>
            position != null
                ? new float3(-position[0], position[1], position[2]) / SCALE_FACTOR
                : float3.Zero;

        public floatQ UnityRotation => floatQ.Euler(0f, MathX.Repeat((-MathX.Rad2Deg * faceAngle) + 180f, 360f) - 180f, 0f);
    ...
    static void DebugPrintCallback(string str) {
        ResoniteMario64.Msg($"[libsm64] {str}");
    }
    ...
    public static void StaticSurfacesLoad(SM64Surface[] surfaces) {
        ResoniteMario64.Msg("Reloading all static collider surfaces, this can be caused by the Game Engine " +
    ...
    public static uint MarioCreate(float3 marioPos) {
        return sm64_mario_create(marioPos.x, marioPos.y, marioPos.z);
    }

    public static SM64MarioState MarioTick(uint marioId, SM64MarioInputs inputs, float3[] positionBuffer,
        float3[] normalBuffer, float3[] colorBuffer, float2[] uvBuffer, out ushort numTrianglesUsed) {
