...
public static class SM64Constants
{
    ...
    public static readonly List<ActionFlag> BaseFlags = new List<ActionFlag>
    {
        ActionFlag.Stationary,
        ActionFlag.Moving,
        ActionFlag.Air,
        ActionFlag.Intangible,
        ActionFlag.Swimming,
        ActionFlag.MetalWater,
        ActionFlag.ShortHitbox,
        ActionFlag.RidingShell,
        ActionFlag.Invulnerable,
        ActionFlag.ButtOrStomachSlide,
        ActionFlag.Diving,
        ActionFlag.OnPole,
        ActionFlag.Mario_Hanging,
        ActionFlag.Mario_Idle,
        ActionFlag.Attacking,
        ActionFlag.AllowVerticalWindAction,
        ActionFlag.ControlJumpHeight,
        ActionFlag.AllowFirstPerson,
        ActionFlag.PauseExit,
        ActionFlag.SwimmingOrFlying,
        ActionFlag.WaterOrText,
...
// TODO: Maybe replace this with the InteractType Below...
...
{
    Hoot = (1 << 0),          // 0x00000001
    Grabbable = (1 << 1),     // 0x00000002
    Door = (1 << 2),          // 0x00000004
    Damage = (1 << 3),        // 0x00000008
    Coin = (1 << 4),          // 0x00000010
    Cap = (1 << 5),           // 0x00000020
    Pole = (1 << 6),          // 0x00000040
    Koopa = (1 << 7),         // 0x00000080
    UNKNOWN08 = (1 << 8),     // 0x00000100
    Breakable = (1 << 9),     // 0x00000200
    StrongWind = (1 << 10),   // 0x00000400
    WarpDoor = (1 << 11),     // 0x00000800
    StarOrKey = (1 << 12),    // 0x00001000
    Warp = (1 << 13),         // 0x00002000
    CannonBase = (1 << 14),   // 0x00004000
    BounceTop = (1 << 15),    // 0x00008000
    WaterRing = (1 << 16),    // 0x00010000
    Bully = (1 << 17),        // 0x00020000
    Flame = (1 << 18),        // 0x00040000
    KoopaShell = (1 << 19),   // 0x00080000
    BounceTop2 = (1 << 20),   // 0x00100000
    MRBlizzard = (1 << 21),   // 0x00200000
    HitFromBelow = (1 << 22), // 0x00400000
    Text = (1 << 23),         // 0x00800000
    Tornado = (1 << 24),      // 0x01000000
    Whirlpool = (1 << 25),    // 0x02000000
    ClamOrBubba = (1 << 26),  // 0x04000000
    BbhEntrance = (1 << 27),  // 0x08000000
    SnufitBullet = (1 << 28), // 0x10000000
    Shock = (1 << 29),        // 0x20000000
    IglooBarrier = (1 << 30), // 0x40000000
    UNKNOWN31 = (1 << 31)     // 0x80000000
}
...
{
    SoundPlayer = 0x00,
    SoundPlayerVariation = SoundPlayer | 0x80,
    EventCutsceneCollectStar = 0x01,
    EventCutsceneCollectStarVariation = EventCutsceneCollectStar | 0x80,
    MenuTitleScreen = 0x02,
    MenuTitleScreenVariation = MenuTitleScreen | 0x80,
    LevelGrass = 0x03,
    LevelGrassVariation = LevelGrass | 0x80,
    LevelInsideCastle = 0x04,
    LevelInsideCastleVariation = LevelInsideCastle | 0x80,
    LevelWater = 0x05,
    LevelWaterVariation = LevelWater | 0x80,
    LevelHot = 0x06,
    LevelHotVariation = LevelHot | 0x80,
    LevelBossKoopa = 0x07,
    LevelBossKoopaVariation = LevelBossKoopa | 0x80,
    LevelSnow = 0x08,
    LevelSnowVariation = LevelSnow | 0x80,
    LevelSlide = 0x09,
    LevelSlideVariation = LevelSlide | 0x80,
    LevelSpooky = 0x0A,
    LevelSpookyVariation = LevelSpooky | 0x80,
    EventPiranhaPlant = 0x0B,
    EventPiranhaPlantVariation = EventPiranhaPlant | 0x80,
    LevelUnderground = 0x0C,
    LevelUndergroundVariation = LevelUnderground | 0x80,
    MenuStarSelect = 0x0D,
    MenuStarSelectVariation = MenuStarSelect | 0x80,
    EventPowerup = 0x0E,
    EventPowerupVariation = EventPowerup | 0x80,
    EventMetalCap = 0x0F,
    EventMetalCapVariation = EventMetalCap | 0x80,
    EventKoopaMessage = 0x10,
    EventKoopaMessageVariation = EventKoopaMessage | 0x80,
    LevelKoopaRoad = 0x11,
    LevelKoopaRoadVariation = LevelKoopaRoad | 0x80,
    EventHighScore = 0x12,
    EventHighScoreVariation = EventHighScore | 0x80,
    EventMerryGoRound = 0x13,
    EventMerryGoRoundVariation = EventMerryGoRound | 0x80,
    EventRace = 0x14,
    EventRaceVariation = EventRace | 0x80,
    EventCutsceneStarSpawn = 0x15,
    EventCutsceneStarSpawnVariation = EventCutsceneStarSpawn | 0x80,
    EventBoss = 0x16,
    EventBossVariation = EventBoss | 0x80,
    EventCutsceneCollectKey = 0x17,
    EventCutsceneCollectKeyVariation = EventCutsceneCollectKey | 0x80,
    EventEndlessStairs = 0x18,
    EventEndlessStairsVariation = EventEndlessStairs | 0x80,
    LevelBossKoopaFinal = 0x19,
    LevelBossKoopaFinalVariation = LevelBossKoopaFinal | 0x80,
    EventCutsceneCredits = 0x1A,
    EventCutsceneCreditsVariation = EventCutsceneCredits | 0x80,
    EventSolvePuzzle = 0x1B,
    EventSolvePuzzleVariation = EventSolvePuzzle | 0x80,
    EventToadMessage = 0x1C,
    EventToadMessageVariation = EventToadMessage | 0x80,
    EventPeachMessage = 0x1D,
    EventPeachMessageVariation = EventPeachMessage | 0x80,
    EventCutsceneIntro = 0x1E,
    EventCutsceneIntroVariation = EventCutsceneIntro | 0x80,
    EventCutsceneVictory = 0x1F,
    EventCutsceneVictoryVariation = EventCutsceneVictory | 0x80,
    EventCutsceneEnding = 0x20,
    EventCutsceneEndingVariation = EventCutsceneEnding | 0x80,
    MenuFileSelect = 0x21,
    MenuFileSelectVariation = MenuFileSelect | 0x80,
    EventCutsceneLakitu = 0x22,
    EventCutsceneLakituVariation = EventCutsceneLakitu | 0x80,
    Count,
    None = 0xFFFF
}
...
public enum InteractionSubtype : uint
{
    // INTERACT_WARP
    FadingWarp = 0x00000001,

    // Damaging interactions
    DelayInvincibility = 0x00000002,
    BigKnockback = 0x00000008, /* Used by Bowser, sets Mario's forward velocity to 40 on hit */

    // INTERACT_GRABBABLE
    GrabsMario = 0x00000004,      /* Also makes the object heavy */
    HoldableNpc = 0x00000010,     /* Allows the object to be gently dropped, and sets vertical speed to 0 when dropped with no forwards velocity */
    DropImmediately = 0x00000040, /* This gets set by grabbable NPCs that talk to Mario to make him drop them after the dialog is finished */
    Kickable = 0x00000100,
    NotGrabbable = 0x00000200, /* Used by Heavy-Ho to allow it to throw Mario, without Mario being able to pick it up */

    // INTERACT_DOOR
    StarDoor = 0x00000020,

    //INTERACT_BOUNCE_TOP
    TwirlBounce = 0x00000080,

    // INTERACT_STAR_OR_KEY
    NoExit = 0x00000400,
    GrandStar = 0x00000800,
...
public enum ActionFlag : uint
{
    ...
    Mario_Throwing = 1u << 31,         // 0x80000000 
