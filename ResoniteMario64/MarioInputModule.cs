/*
 * TODO: figure out how to input mario :D
using FrooxEngine;

namespace ResoniteMario64;

public class MarioInputModule : Component {//CVRInputModule {
    ...
    public override void UpdateInput() {
        if (controllingMarios > 0 && InputInterface.GetKeyDown(Key.LeftShift)) {
            canMoveOverride = true;
        }

        if (InputInterface.GetKeyUp(Key.LeftShift)) {
        ...
        if (!CanMove()) {
            ...
            _inputManager.movementVector = float3.zero;
            ...
            if (MarioCameraMod.IsControllingAMario(out var mario) && MarioCameraMod.IsFreeCamEnabled()) {
                _inputManager.lookVector = float2.zero;
