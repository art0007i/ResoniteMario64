...
public class MarioCameraMod : ICameraVisualMod, ICameraVisualModRequireUpdate {
    ...
    public void Update() {
        ...
        if (_currentMario.IsFirstPerson()) {
            ...
            var targetPosition = transform.position + float3.up * _elevation / multiplier - transform.forward * _distance / multiplier;
            // Damp the camera movement to make it smooth
            _camera.transform.position = float3.SmoothDamp(_camera.transform.position, targetPosition, ref _velocity, 0.3f);
            // Look at the target
            ...
            _currentPitch = MathX.Clamp(_currentPitch, _minPitch, _maxPitch);

            // Calculate the combined rotation based on the yaw and pitch values
            var combinedRotation = floatQ.Euler(_currentPitch, _currentYaw, 0);

            // Calculate the camera offset with the new combined rotation
            var cameraOffset = combinedRotation * new float3(0, 0, _distance);
            ...
            n = floatQ.AngleAxis(MarioInputModule.Instance.horizontal, float3.up ) * n;
            n += m;
